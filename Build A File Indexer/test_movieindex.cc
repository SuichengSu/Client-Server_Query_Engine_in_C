...
TEST(Movie, CreateFromRow) {
  ...
  // TODO(done): After changing Movie to handle multiple genres, you may need to change this line. 
  for (int i = 0; i < 10; i++) {
  	ASSERT_EQ(m1->genres[i], nullptr);
  }
  ASSERT_EQ(-1, m1->runtime); 
            
  
  // TODO(done): Create from a improper row
  char row2[1000];
  strcpy(row2, movie_row_2);
...
TEST(MovieIndex, AddMovieToIndex) {
  ...
  AddMovieToIndex(index, m1, Type); 
  // TODO(done): What's supposed to happen here? 
  // Write code to check it.
  ASSERT_EQ(NumElemsInHashtable(index), 1);
  // The number of elements should be the same. Nothing will happen since the old_kv is the same.
  
  // Add another movie to the index (same IndexType)
  AddMovieToIndex(index, m2, Type); 
  
  // Add another movie (different IndexType)
  // TODO(done): Check result is correct. 
