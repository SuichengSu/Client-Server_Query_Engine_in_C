...
// Assumes that kvp is NOT allocated on the heap. 
// 
// If there is already a KeyValuePair with the same key in the Hashtable, 
// the new kvp gets put in, but old_kvp gets populated with the kvp that is 
// currently in the hashtable, so it can be handled properly by the calling 
// function. Return the relevant code as noted below. 
//
// INPUT:
